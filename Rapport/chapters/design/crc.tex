...
The execution of the division itself is done by binary dividing the message by the generator polynomial, without carries. This means that any bit $x^k$ is not affected by the bits $x^{k+1}$ or $x^{k-1}$. This kind of division propagates to be an \textit{exclusive or} function, as seen in \tabref{binarydivision}.
