...
Exponential backoff is used in the solution to handle transmitting conflicts. In the solution, a response is often wanted when sending data, and exponential backoff can be used to make sure that this arrives at some point. For example, this is used when sending data and waiting for an acknowledgment.\todo{any other places? Name all if few.}
