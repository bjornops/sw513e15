\clearpage
\section{Data verification} \label{cha:dutuVurufucutiun}
%Why checksum
...
%What is a checksum
A checksum is a unique value that can be used to verify that a message is received correctly \cite{datapakke}. The checksum is based on all parts of the message and unique to the length of the checksum. 
...
\subsection*{MD5}
MD5 is a widespread checksum algorithm. It produces a 128 bit checksum, which is 16 bytes. MD5 is designed to run efficiently on 32 bit processors, which is a disadvantage on the Arduino boards used in the solution, since these posses an 8 bit processor. The 16 byte checksum uses half of the possible payload of the nRF24L01, which is too much. The MD5 algorithm is therefore discarded as the checksum generator for this project \cite[p.~308]{boyles2010ccna}.

\subsection*{Cyclic redundancy check}
Cyclic redundancy check is a checksum algorithm, hereafter referenced to as \textit{CRC}. CRC exists in several forms, based on the parameters, such as the checksum size and internal algorithm parameters. CRC can be used to detect either one or more 'single bit error' and burst errors \cite[p.~31]{elahi2001network}. Due to the small checksum size and the error detection properties, CRC is chosen as the checksum generator.

\subsection{Computation of a CRC checksum}\label{cha:crcComp}
...
A message $M$ is viewed as a polynomial with binary coefficients, by treating each character in $M$ bit-wise. This makes for a relatively large number, but the decimal equivalent is irrelevant for the computation. The length of M in bits is called $m$, which as exemplified in table \ref{tab:string} is 24.
