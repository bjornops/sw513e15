\section{Platforms}
The suitable platforms for this project are examined in the following section. These platforms are capable of running application code and reading or writing to ports available on the board. These ports can connect to components such as sensors and actuators.
...
How the Arduino handles or reacts to input is implemented in the developed code uploaded to the Arduino board by using the Arduino IDE.
...
The Raspberry Pi got a set of pins for connecting to external hardware. There are 4 different pin types available; 2 5v power, 2 3.3v power, 5 ground and 17 GPIO. On \figref{abPins} the placement of the pins is shown. 
...
Because of the computing power and memory of a Raspberry pi, a Linux OS is usually installed on a SD card and then plugged into the Pi for executing. A Raspberry Pi also has a GPU, video output, and USB port.
