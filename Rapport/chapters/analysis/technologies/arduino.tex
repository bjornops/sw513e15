...
Raspberry Pi is a series of single board computers. The Raspberry Pi series contains some rather powerful controllers compared to their credit card sized boards, which makes them great\todo{remove commercial} as small processing units.
...
Because of the computing power and memory of a Raspberry Pi, a Linux OS is usually\todo{source} installed on a SD card and then plugged into the Pi for executing. A Raspberry Pi also has a GPU, video output, and USB port.
